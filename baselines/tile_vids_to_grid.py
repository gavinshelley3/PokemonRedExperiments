...
def run_ffmpeg_grid(
    out_path, files, screen_res_str, full_res_string, gx, gy, short_test=False
):
    cmd = ["ffmpeg"]
    for file in files:
        cmd.append("-i")
...
def make_script(path):
    ...
    return run_ffmpeg_grid(
        (sess_dir / sess_dir.name).with_suffix(".mp4"),
        all_files,
        "160x144",
        "1280x720",
        8,
...
def make_outer_script(out_file, paths):
    return run_ffmpeg_grid(
        out_file, paths, "1280x720", "10240x5760", 8, 8, short_test=False
    )
...
if __name__ == "__main__":
    ...
    if inner_mosaic:
        ...
        all_sessions = list(outer_dir.glob("session/session_*"))
        scripts = [make_script(sess) for sess in all_sessions]
        for script, sess in zip(scripts, all_sessions):
            out_file = Path(
                outer_dir / Path("parallel_scripts") / sess.with_suffix(".sh").name
            )
            write_file(out_file, script)
    else:
        base = Path("grid_renders")
        all_input_vids = list(base.glob("session/session_*/session_*.mp4"))
